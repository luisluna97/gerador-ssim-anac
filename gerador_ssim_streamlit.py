...
def ajustar_formato_ssim(linha_ssim):
    ...
    try:
        ...
        if match:
            ...
            # Remover 2 espaços e inserir o código da CIA
            if len(espacos_originais) >= 10:  # Garantir que tem espaços suficientes
                # Calcular nova distribuição: remover 2 espaços, inserir código + espaço
                espacos_restantes = len(espacos_originais) - 2 - len(codigo_cia) - 1
                
                if espacos_restantes > 0:
                    # Distribuir espaços: mais na frente, alguns no meio
                    espacos_antes = espacos_restantes - 7
                    espacos_meio = 7
                    
                    if espacos_antes > 0:
                        novo_formato = f"{' ' * espacos_antes}{codigo_cia}{' ' * espacos_meio}{codigo_cia} {numero_voo}"
        ...
        if match_espacos:
            ...
            if resto_linha.startswith(f"{codigo_cia} "):
                ...
                if len(espacos_encontrados) >= 10:
                    espacos_restantes = len(espacos_encontrados) - 2 - len(codigo_cia) - 1
                    ...
                    if espacos_restantes > 0:
                        ...
                        if espacos_antes > 0:
                            novo_formato = f"{' ' * espacos_antes}{codigo_cia}{' ' * espacos_meio}"
        ...
        print(f"Erro no ajuste SSIM: {e}")  # Para debug
        return linha_ssim
