...
def ajustar_formato_ssim(linha_ssim):
    ...
    try:
        ...
        # Encontrar a posição onde inserir o código da CIA
        # Procurar por uma sequência de muitos espaços antes do código do voo
        # Padrão: muitos espaços + código_cia + espaço + número do voo
        padrao_busca = f" {codigo_cia} \\d{{4}}"
        import re
        match = re.search(padrao_busca, linha_ssim)
        
        if match:
            # Encontrou o padrão, vamos inserir o código da CIA antes
            posicao_insercao = match.start()
            
            # Contar quantos espaços há antes dessa posição
            espacos_antes = 0
            pos = posicao_insercao - 1
            while pos >= 0 and linha_ssim[pos] == ' ':
                espacos_antes += 1
                pos -= 1
            
            # Se há pelo menos 10 espaços, vamos fazer o ajuste
            if espacos_antes >= 10:
                # Dividir a linha em 3 partes: antes, meio, depois
                parte_antes = linha_ssim[:posicao_insercao - espacos_antes]
                parte_depois = linha_ssim[posicao_insercao:]
                
                # Criar a parte do meio: remover 2 espaços e adicionar código da CIA
                espacos_meio = espacos_antes - 2 - len(codigo_cia)
                if espacos_meio > 0:
                    parte_meio = ' ' * espacos_meio + codigo_cia + ' ' * (len(codigo_cia) - len(codigo_cia) + 1)
                    linha_ajustada = parte_antes + parte_meio + parte_depois
                    return linha_ajustada
        
        # Se não conseguiu fazer o ajuste automático, fazer manualmente
        # Procurar por sequência de 15+ espaços
        match_espacos = re.search(r' {15,}', linha_ssim)
        if match_espacos:
            inicio_espacos = match_espacos.start()
            fim_espacos = match_espacos.end()
            
            # Dividir a linha
            parte_antes = linha_ssim[:inicio_espacos]
            parte_depois = linha_ssim[fim_espacos:]
            
            # Calcular quantos espaços usar
            total_espacos = fim_espacos - inicio_espacos
            espacos_restantes = total_espacos - 2 - len(codigo_cia)
            
            if espacos_restantes > 0:
                # Distribuir os espaços: alguns antes, alguns depois
                espacos_antes_cia = espacos_restantes - 7  # Deixar 7 espaços depois
                espacos_depois_cia = 7
                
                if espacos_antes_cia > 0:
                    parte_meio = ' ' * espacos_antes_cia + codigo_cia + ' ' * espacos_depois_cia
                    linha_ajustada = parte_antes + parte_meio + parte_depois
                    return linha_ajustada
